...
def list_cards():
    ...
    folder_obj = Folder.query.get(folder_id_int) if folder_id_int else None
    is_deck_folder = bool(
        folder_obj
    ...
    # Base query
    query = Card.query
    include_param = (request.args.get("include_proxies") or "").strip().lower()
    include_proxies = folder_is_proxy
    if include_param in {"1", "true", "yes"}:
        include_proxies = True
    elif include_param in {"0", "false", "no"}:
    ...
    if typal and needs_typal_fallback:
        ...
        if keep_ids:
            ...
            return render_template(
                ...
                page=1,
                per=per,
                include_proxies=include_proxies_flag,
                pages=1,
                prev_url=None,
                next_url=None,
                page_urls=[],
...
def bulk_move_cards():
    ...
    if moved:
        ...
        record_audit_event(
            "cards_bulk_move",
            {"target_folder": target_folder.id, "count": moved, "card_ids": card_ids[:50]},
        )
        folder_name = target_folder.name or f"Folder {target_folder.id}"
        flash(f"Moved {moved} card{'s' if moved != 1 else ''} to {folder_name}.", "success")
    else:
        flash("Selected cards are already in that folder.", "info")
...
def api_deck_insight(deck_id: int):
    folder = Folder.query.get_or_404(deck_id)
...
@views.route("/deck-synergy")
def deck_synergy():
    """
    Commander-style recommendations inspired by EDHREC:
    - Select a deck (folder) to analyse its composition
    - Highlight core role counts (ramp, draw, interaction, wipes)
    - Surface theme signals (tokens, +1/+1 counters, etc.)
    - Offer curated card suggestions filtered by deck colours
    """
    ...
    analysis = None
    errors: list[str] = []
    selected_folder_obj: Folder | None = None

    if folder_id:
        selected_folder_obj = Folder.query.get(folder_id)
        if not selected_folder_obj:
            errors.append("Deck not found. Please pick another list to analyse.")
        else:
            try:
                analysis = analyze_deck(folder_id)
                if analysis and not analysis.get("roles"):
                    errors.append("This deck does not have enough data to analyse yet.")
            except Exception as exc:
                current_app.logger.exception("Deck synergy analysis failed for folder %s", folder_id)
                errors.append(f"Unable to analyse this deck: {exc}")

    role_alerts = []
    theme_count = 0
    hover_lookup: Dict[str, str] = {}
    if analysis:
        role_alerts = [role for role in analysis.get("roles", []) if role.get("status") == "needs_attention"]
        theme_count = len(analysis.get("themes", []))

        def _collect_hover(payload: object) -> None:
            if not payload:
                return
            if isinstance(payload, dict):
                ...
                if name and hover:
                    lowered = name.lower()
                    if lowered not in hover_lookup:
                        hover_lookup[lowered] = hover
        ...
        _collect_hover(analysis.get("deck"))
        _collect_hover(analysis.get("roles"))
        _collect_hover(analysis.get("themes"))
        _collect_hover(analysis.get("edhrec"))
        _collect_hover(analysis.get("deck_card_rows"))
        _collect_hover(analysis.get("analysis_cards"))

    return render_template(
        "decks/deck_synergy.html",
        deck_options=deck_options,
        selected_id=folder_id,
        selected_folder=selected_folder_obj,
        analysis=analysis,
        errors=errors,
        role_alert_count=len(role_alerts),
        theme_count=theme_count,
        hover_lookup=hover_lookup,
    )


@views.post("/deck-synergy/<int:folder_id>/playground")
def deck_synergy_playground(folder_id: int):
    """Create a Build-A-Deck playground clone from the selected deck."""
    source = Folder.query.get_or_404(folder_id)
    if source.is_collection:
        flash("Collection folders cannot be cloned into a playground.", "warning")
        return redirect(url_for("views.deck_synergy", folder_id=folder_id))
    if (source.category or Folder.CATEGORY_DECK) == Folder.CATEGORY_BUILD:
        flash("This deck is already a Build-A-Deck list.", "info")
        return redirect(url_for("views.build_a_deck", folder_id=source.id))

    try:
        playground = _clone_deck_to_playground(source)
        db.session.commit()
    except Exception as exc:
        db.session.rollback()
        current_app.logger.exception("Failed to create playground deck from %s", folder_id)
        flash(f"Unable to create playground: {exc}", "danger")
        return redirect(url_for("views.deck_synergy", folder_id=folder_id))

    origins = session.setdefault("playground_origins", {})
    origins[str(playground.id)] = source.id
    session.modified = True

    flash(
        f'Playground deck "{playground.name}" created. You can tweak it freely.',
        "success",
    )
    return redirect(url_for("views.build_a_deck", folder_id=playground.id))


@views.route("/api/decks/<int:folder_id>/analysis")
def api_deck_analysis(folder_id: int):
    try:
        analysis = analyze_deck(folder_id)
    except ValueError:
        return jsonify({"error": "Deck not found"}), 404
    except Exception as exc:
        current_app.logger.exception("Deck analysis API failed for %s", folder_id, exc_info=True)
        return jsonify({"error": "Unable to analyse this deck"}), 500
