...
def _cached_synergy_summary(folder_id: int) -> Optional[Dict[str, Any]]:
    """Lightweight cached snapshot used for inline deck insights."""
    details = analyze_deck(folder_id)
    return _synergy_inline_summary(details)


def _synergy_inline_summary(details: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    if not details:
        return None

    deck_info = details.get("deck") or {}
    color_identity = deck_info.get("color_identity") or {}
    color_letters = (color_identity.get("letters") or "").upper()
    color_list = [c for c in color_letters if c.isalpha()]

    analysis_cards = details.get("analysis_cards") or []
    analysis_card_map: Dict[int, Dict[str, Any]] = {}
    card_roles: Dict[int, List[str]] = {}
    role_cards: Dict[str, List[int]] = {}
    for entry in analysis_cards:
        ...
        try:
            ...
            continue
        role_keys = [str(r) for r in (entry.get("roles") or []) if r]
        if not role_keys:
            continue
        card_roles[card_id_int] = role_keys
        for key in role_keys:
            role_cards.setdefault(key, []).append(card_id_int)
        analysis_card_map[card_id_int] = entry

    roles = details.get("roles") or []
    role_entries: List[Dict[str, Any]] = []
    for role in roles:
        role_key = str(role.get("key") or "")
        label = role.get("label") or (role_key or "").replace("_", " ").title()
        current = int(role.get("current") or 0)
        cards_payload: List[Dict[str, Any]] = []
        card_ids_for_role = sorted(
            {cid for cid in role_cards.get(role_key, [])},
            key=lambda cid: (
                (analysis_card_map.get(cid, {}).get("name") or "").lower(),
                cid,
            ),
        )
        for card_id in card_ids_for_role:
            card_entry = analysis_card_map.get(card_id) or {}
            best_synergy = card_entry.get("best_synergy")
            best_synergy_value: Optional[int]
            try:
                best_synergy_value = int(round(float(best_synergy))) if best_synergy is not None else None
            except (TypeError, ValueError):
                best_synergy_value = None
            cards_payload.append(
                {
                    "id": card_id,
                    "name": card_entry.get("name"),
                    "quantity": int(card_entry.get("quantity") or 0) or 1,
                    "hover_image": card_entry.get("hover_image"),
                    "best_synergy_percent": best_synergy_value,
                    "edhrec": card_entry.get("edhrec") or {},
                }
            )
        role_entries.append(
            {
                ...
                "current": current,
                "cards": cards_payload,
                "recommendations": list(role.get("recommendations") or []),
                "edhrec_recommendations": list(role.get("edhrec_recommendations") or []),
            }
        )
    ...
    highlighted_roles = role_entries

    coverage = (details.get("edhrec") or {}).get("coverage") or {}
    coverage_summary = {
        "present_average_synergy_percent": coverage.get("present_average_synergy_percent"),
        "present_average_synergy": coverage.get("present_average_synergy"),
        "present_count": coverage.get("present_count"),
        "missing_candidates": coverage.get("missing_candidates"),
    }

    tag_synergy = details.get("tag_synergy") or {}
    theme_summary = None

    tag_focus = None
    if tag_synergy:
        tag_focus = {
            "label": tag_synergy.get("label") or deck_info.get("tag_label") or deck_info.get("tag"),
            "score": tag_synergy.get("score"),
        }
    ...
    return {
        "deck": {
            ...
            "color_label": color_identity.get("label") or ("Colorless" if not color_list else None),
            "color_letters": color_letters,
            "color_list": color_list,
            "total_cards": deck_info.get("total_cards"),
            "unique_cards": deck_info.get("unique_cards"),
            "tag_label": deck_info.get("tag_label") or deck_info.get("tag"),
        },
        "roles": highlighted_roles,
...
def _folder_detail_impl(folder_id: int, *, allow_shared: bool = False, share_token: str | None = None):
    ...
    if not folder.is_collection:
        try:
            synergy_summary = _cached_synergy_summary(folder_id)
        except Exception as exc:
            current_app.logger.warning("Inline synergy snapshot failed for folder %s: %s", folder_id, exc)
            synergy_error = "Synergy insights are temporarily unavailable."
...
def set_folder_proxy(folder_id: int):
    ...
    (
        Card.query.filter(Card.folder_id == folder.id)
        .update({Card.is_proxy: desired}, synchronize_session=False)
    )
    _safe_commit()
