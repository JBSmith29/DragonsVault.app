...
from models.role import Role, SubRole, CardRole, CardSubRole, OracleRole, OracleDeckTag, OracleEvergreenTag
    ...
    if request.method == "POST":
        ...
        if action == "refresh_oracle_tags":
            ...
            try:
                ...
                flash("Oracle deck tags and evergreen keywords refreshed.", "success")
            except Exception as exc:
        ...
        if action == "refresh_oracle_enrichment_full":
            ...
            try:
                ...
                flash("Oracle roles, keywords, typal tags, deck tags, and evergreen keywords refreshed.", "success")
            except Exception as exc:
...
def admin_oracle_tags():
    ...
    deck_query = OracleDeckTag.query
    evergreen_query = OracleEvergreenTag.query
    if q:
        like = f"%{q}%"
        name_match = db.session.query(OracleRole.oracle_id).filter(OracleRole.name.ilike(like)).subquery()
    ...
    deck_rows = deck_query.order_by(OracleDeckTag.tag, OracleDeckTag.oracle_id).limit(500).all()
    evergreen_rows = evergreen_query.order_by(OracleEvergreenTag.keyword, OracleEvergreenTag.oracle_id).limit(500).all()

    oracle_ids = {row.oracle_id for row in deck_rows} | {row.oracle_id for row in evergreen_rows}
    name_map = {}
    if oracle_ids:
        names = (
