...
from core.domains.users.services import audit
from core.shared.utils.time import utcnow

from core.routes.base import views, limiter_key_user_or_ip, _safe_next_url
...
def login():
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        audit.record_audit_event("login", {"email": user.email})
...
@login_required
def logout():
    audit.record_audit_event("logout", {"email": current_user.email})
    logout_user()
    session.clear()
...
def register():
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        audit.record_audit_event("user_registered", {"email": email, "username": username})
...
def manage_api_token():
    ...
    if action == "create":
        ...
        db.session.commit()
        audit.record_audit_event("api_token_issued", {"hint": issued_token[-8:]})
        flash("New API token generated. Copy it now; it is shown only once.", "success")
    elif action == "revoke":
        current_user.clear_api_token()
        db.session.commit()
        audit.record_audit_event("api_token_revoked", {})
        ...
        if not current_user.check_password(current_password):
            ...
            db.session.commit()
            audit.record_audit_event("password_updated", {"method": "self_service"})
